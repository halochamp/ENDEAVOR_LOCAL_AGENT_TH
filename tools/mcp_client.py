...
ClientSession; imported lazily so a missing package fails as a clean [error],
not an import-time crash for every tool.
"""
...
def _load_dynamic_servers() -> dict:
...
def _save_dynamic_servers(servers: dict) -> None:
    """Atomic write (tmp file + os.replace) — avoids a torn/partial JSON file
    if the process dies mid-write, same shape as tools/write_file.py."""
    path = _dynamic_path()
