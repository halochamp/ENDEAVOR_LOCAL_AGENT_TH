...
def _read_file_impl(
    ...
    try:
        ...
        if suffix in _IMAGE_EXT:
            return f"[error] {path} is an image file — use read_image to read it"
        ...
        if b"\x00" in raw[:8192]:
            return (f"[error] {path} is a binary file, not readable as text — "
                    f"use read_image if it's an image")
...
def read_file(
    ...
    read just that section directly instead of the whole file. This is the preferred
    way to inspect a known section; fall back to bash (`rg -n`, `find`, `rg --files`) only when the section's
    location isn't known yet and needs to be searched for first. line_start/line_end
    have no effect on PDF/DOCX/XLSX/XLS, audio/video, or image files.
    Code files over the size limit return a structure map (symbols + line numbers)
    unless a line range is given, in which case that exact range is returned instead.
    ...
    image-only PDFs (no extractable text) fall back to on-device Apple Vision OCR
    automatically.
    PDF PAGE RANGE: pass page_start/page_end (1-indexed, inclusive, PDF only) to
    read exact pages directly instead of a sampled/truncated read — works for both
    extractable-text and scanned/OCR PDFs. Each call stops at a clean page boundary
    (never mid-page) once either 30 pages or the output budget is reached, and always
    states the exact page_start to pass next in a trailing [note: ...] line — keep
    calling with that value until the note stops appearing. Dense documents return
    far fewer than 30 pages per call — trust the note, don't assume 30. Mutually
    exclusive with line_start/line_end, the search filters below, and doc_mode;
    user_query is ignored when page_start/page_end is set (the range is read in
    full, nothing is pinned/sampled — pass user_query without a page range instead
    if you want that). page_start/page_end also bypasses the file-size gate below.
    Audio/video (m4a/mp3/wav/aiff/mp4/mov, etc.) are transcribed on-device (Thai) via
    native Apple Speech — the full raw transcript is saved to a .md file in workspace
    and a summary is returned here; first-ever call on a machine needs Siri enabled in
    System Settings and a one-time consent click.
    For plain-text/code files, and also for PDF/DOCX/XLSX/XLS after conversion to
    markdown, pass one search mode to return only matching sections with surrounding
    context_lines (default 3):
      ...
      regex_flags="ims"            — regex flags: i=ignorecase, m=multiline, s=dotall
      whole_word=true              — word-boundary match for contains / contains_any / contains_all
    Document-only structured search: set doc_mode to narrow the search target
    inside a converted document:
      doc_mode="heading"           — match markdown headings only
      doc_mode="section"           — return the whole section when the heading or any line inside that section matches
      doc_mode="row"               — match markdown table rows only
      doc_mode="cell"              — match individual markdown table cells
    PDF TABLE EFFICIENCY: after a broad/sampled document result, retry ONCE with
    contains/regex + context_lines. Never repeat only user_query or add line ranges;
    never render/crop/read_image while extractable text exists. Use read_image only
    when read_file reports that the PDF has no extractable text.
    Quick examples:
      read_file("notes.md", contains="prefill", context_lines=2)
      ❌ read_file("plan.pdf", user_query="July") again, then render/crop pages
      ✅ read_file("plan.pdf", contains="July", context_lines=6), then answer
      read_file("report.docx", contains="South", doc_mode="section")
      read_file("sheet.xlsx", contains_all=["Apple","East"], doc_mode="row")
      read_file("sheet.xlsx", contains="North", doc_mode="cell")
    Search modes are mutually exclusive with each other and with line_start/line_end.
    doc_mode requires one search mode, and is only valid for document files.
    Files larger than READ_FILE_MAX_BYTES (default 50 MB) are rejected — use bash (`rg -n`, `find`, `rg --files`) to
    target sections (audio/video get a separate, larger limit). Exception: a PDF read
    with page_start/page_end is exempt from this gate, since it only ever touches the
    requested pages — use page_start/page_end on an oversized PDF instead of giving up.
...
def _transcribe_to_state(p: Path, path: str, user_query: str = "") -> str:
    ...
    summary = summarize(sampled, url=path, user_query=user_query)
    ...
    *,
    ...
    for sec in matched_sections:
        ...
        if overshadowed:
            ...
            # too — that content would otherwise vanish entirely.
