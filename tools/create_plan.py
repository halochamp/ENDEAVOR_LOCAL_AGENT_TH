...
def format_plan(steps: list[str]) -> str:
    ...
    return (
        ...
        f"  search/find/ค้นหา → web_search\n"
        f"  browse URL → browse_url\n"
        f"  browse multiple URLs in parallel → batch_browse\n"
        f"  iterative multi-source loop (search+read many items) → tool_loop\n"
        f"  read file → read_file\n"
        f"  analyze numbers → python_exec\n"
