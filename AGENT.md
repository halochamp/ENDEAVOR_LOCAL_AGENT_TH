...
- `agent_server.py` — authenticated WebSocket/REST backend for Electron and explicit custom clients; also publishes self-contained host telemetry in UI order CPU/GPU/RAM/TOK/network, with a model-agnostic rolling 5s generation token/sec meter and no private Server Monitor dependency or machine-specific path;
