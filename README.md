...
- ปุ่ม **📌 Pin File** สร้าง working set ต่อเนื่องได้สูงสุด 10 ไฟล์จาก Workspace ต่อ Electron session. Pin อยู่ต่อหลัง Send และ `/clear` แต่ไม่ persist ข้ามการปิด/เปิด Electron ใหม่. ทุก normal query backend จะตรวจ canonical path/Workspace/existence/readability ใหม่ แล้ว graph จะ **อ่าน Pin ทุกไฟล์ก่อน reasoning**: เอกสารใช้ `read_file` แบบ question-aware และ batch สูงสุด 4 ไฟล์ต่อ call (10 ไฟล์ = 4+4+2), รูปใช้ progressive `read_image(..., detail="overview")` สูงสุด 10 ภาพ; OCR/zoom เป็น follow-up เมื่อจำเป็น. Source เดียวกันเลือกหลายทางจะใช้ลำดับ **Pin > Attach > @mention** และอ่านเพียงครั้งเดียวต่อ turn; ไฟล์ Pin ที่หาย/ย้าย/อ่านไม่ได้จะรายงาน `อ่านสำเร็จ X/Y` แทนการแกล้งทำว่าอ่านครบ
