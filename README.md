...
- แถบข้อความเหนือช่องพิมพ์แสดง `CPU · GPU · RAM · NET · TOK` แบบ real-time แทนการแสดง phase ซ้ำกับใน chat. Telemetry มาจาก Agent TH เอง ไม่พึ่ง Server Monitor/private repo และไม่ hard-code path/สเปกของเครื่องผู้พัฒนา: CPU/RAM/Network ใช้ `psutil`, GPU ตรวจ capability ของเครื่องที่รันจริง (`ioreg` บน macOS หรือ `nvidia-smi` ถ้ามี) และถ้าอ่านไม่ได้จะแสดง `GPU —` โดยไม่กระทบ Agent. `TOK` คือ token/sec แบบ rolling average 5 วินาทีจาก streaming generation callback จริงของ LLM ที่กำลังทำงาน จึงไม่ fix ชื่อ model และไม่ประมาณจากจำนวนตัวอักษร; เมื่อไม่มี active generation จะแสดง `0.0 t/s`. CPU/GPU/RAM/NET sample ทุก 5 วินาทีตามปกติ แต่ bar publish ทุก 1 วินาทีเพื่อให้ `TOK` ตอบสนองระหว่าง generation โดยไม่เพิ่ม system/GPU polling เป็น 1 Hz. Backend ส่ง event นี้เฉพาะ WebSocket ที่ประกาศ `transport=desktop` เพื่อไม่เพิ่ม unsolicited telemetry ให้ custom client ทั่วไป
