...
def print_edit_access_menu(state: dict) -> None:
    ...
    print()
    print(f" {C_DIM}{'─' * (WIDTH - 2)}{R}")
    print(f"   {BOLD}{C_HEADER}Approved Edit Folders / Active Workspace{R}")
    print(f"   {C_META}Persistent approvals:{R} {C_AGENT}{len(folders)} / {max_folders}{R}")
    for idx, folder in enumerate(folders, 1):
        print(f"      {C_ORANGE}{idx}.{R} {C_AGENT}{folder}{R}")
    print(f"   {C_META}Active Workspace:{R} {C_AGENT}{focus or 'Not set'}{R}")
    print(f"   {C_DIM}Active Workspace is temporary and never auto-adds to persistent approvals.{R}")
    print()
    print(f"   {C_ORANGE}[1]{R} เพิ่ม Approved Edit Folder")
    print(f"   {C_ORANGE}[2]{R} ลบ Approved Edit Folder")
    print(f"   {C_ORANGE}[3]{R} ตั้ง Active Workspace")
    print(f"   {C_ORANGE}[4]{R} ล้าง Active Workspace")
...
def print_runtime_settings_menu(settings: dict) -> None:
    ...
    mode = str(settings.get("server_mode") or "standalone")
    mode_label = "Shared MAX" if mode == "shared_max" else "Standalone"
    port = int(settings.get("server_port") or 8085)
    print()
    print(f" {C_DIM}{'─' * (WIDTH - 2)}{R}")
    print(f"   {BOLD}{C_HEADER}Model / Think Budget / Port{R}")
    print(f"   {C_META}Mode:{R}  {C_AGENT}{mode_label}{R}")
    print(f"   {C_META}Model:{R} {C_AGENT}{model_label}{R}{C_DIM}{lock_note}{R}")
    print(f"   {C_META}Think:{R} {C_AGENT}{budget_label} · {budget}{R}")
    print(f"   {C_META}Port:{R}  {C_AGENT}:{port}{R}")
    print()
    print(f"   {C_ORANGE}[1]{R} เปลี่ยน Model")
    print(f"   {C_ORANGE}[2]{R} เปลี่ยน Think Budget")
    print(f"   {C_ORANGE}[3]{R} เปลี่ยน Model Server Port")
    print(f"   {C_ORANGE}[b]{R} Back")
    print(f" {C_DIM}{'─' * (WIDTH - 2)}{R}")
    print()
